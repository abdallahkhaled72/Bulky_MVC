...
namespace BulkyBookWeb.Areas.Admin.Controllers
{
   ...
   [Authorize(Roles = SD.Role_Admin)]
    public class ProductController : Controller
    {
        ...
        [HttpPost]
        public IActionResult Upsert(ProductVM productVM, List<IFormFile> files) // IFormFile uses to apply uploading files
        {
            if (ModelState.IsValid)
            {
                ...
                if (files != null)
                {
                    foreach(IFormFile file in files)
                    {
                        ...
                        // check if the product image is not null and if it is null u add image
